...
public interface MaintenanceAssetLeasedJpa extends JpaRepository<MaintenanceAssetLeasedEntity, String> {
    ...
    List<MaintenanceAssetLeasedEntity> findAll(Pageable pageable, @Param("from") Timestamp from, @Param("to") Timestamp to, @Param("searchText") String searchText, @Param("status") Integer status);
    @Query("SELECT COUNT(m) FROM MaintenanceAssetLeasedEntity m WHERE m.isDeleted = false " +
            "AND (:searchText is null or m.client.fullName like %:searchText%) " +
            "AND ((:from is null or (m.startedAt >= :from)) and (:to is null or (m.startedAt <= :to))) " +
            "AND (:status is null or m.status = :status)")
    Long countTotal(@Param("from") Timestamp from, @Param("to") Timestamp to, @Param("searchText") String searchText, @Param("status") Integer status);
    @Modifying
    @Transactional
