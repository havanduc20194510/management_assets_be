...
public interface MaintenanceAssetJpa extends JpaRepository<MaintenanceAssetEntity, MaintenanceAssetLeasedKey> {
    ...
    @Query("SELECT COUNT(m)>0 FROM MaintenanceAssetEntity m WHERE m.id.assetLeasedId = :assetLeasedId")
    Boolean existedMaintenanceAssetLeased(@Param("assetLeasedId") Long assetLeasedId);
}
